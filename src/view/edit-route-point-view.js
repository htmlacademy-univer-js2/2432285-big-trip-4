...
function createEventTypesList(currentType, isDisabled, pointID) {
...
function createEditRoutePointTemplate(state, offersByType, destinations, mode) {
  ...
  const { id, basePrice, dateFrom, dateTo, offers, type, destination } = routePoint;
  const currentDestination = destination !== null && typeof destination !== 'undefined' ? destinations.find((dest) => dest.id === destination) : DEFAULT_DESTINATION;
    ...
    <li class="trip-events__item">
      <form class="event event--edit" action="#" method="post">
        <header class="event__header">
          ${createEventTypesList(type, isDisabled, id)}
          <div class="event__field-group event__field-group--destination">
            ...
            ${createDestinationList(destinations, id)}
          </div>
          ...
          <div class="event__field-group event__field-group--price">
            ...
            <input class="event__input event__input--price" id="event-price-${id}" type="number" name="event-price" value="${basePrice}" min="0" max="100000" ${isDisabled ? 'disabled' : ''}>
          </div>
